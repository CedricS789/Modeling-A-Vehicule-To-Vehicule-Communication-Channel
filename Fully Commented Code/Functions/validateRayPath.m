...
%                           for example no intersection or obstruction.
    ...
    K = length(all_images_pos) - 1;                                                    % The number of reflections is one less than the total number of sources (original + images).
    ...
    for k = K:-1:1                                                                        % Trace path segments backwards, from the final reflection toward the first.
        ...
        for j = 1:length(walls)                                                            % Iterate through all walls in the environment to check for blockages.
            ...
            is_next_segments_wall = (k < K && j == processed_wall_indices(k+1));          % And the wall of the next reflection (the previous in this backward trace).
