...
function trace_recursive(ax, tx_pos, RX_pos, w, n, k)
    ...
    % ---- Trace and plot rays for the current reflection order 'n' ----
    for ray_type = 1:2 % Two ray families: starting top or starting bottom
        
        % Generate the sequence of n image sources for the current ray
        images = cell(n, 1);
        current_pos = tx_pos;
        for i = 1:n
        ...
        % Plot the full ray if it's valid
        if is_ray_valid
