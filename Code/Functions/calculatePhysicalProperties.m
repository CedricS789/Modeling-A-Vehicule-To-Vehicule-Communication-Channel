...
%   ray_data            - A struct containing all calculated physical properties.
    ...
    K = size(ray_coords, 1) - 2;                                    % The number of reflections is the vertex count minus the TX and RX.
    ...
    for n = 1:(K + 1)
        start_segment = ray_coords(n,:);
        end_segment = ray_coords(n+1,:);
        ...
        if is_reflection_segment
            ...
            reflecting_wall = walls(wall_indices_path(n));
            ...
            sin_theta_n = 1 - cos_theta_n^2;                                   % An intermediate term for the Fresnel equations, derived from sin² + cos² = 1.
    ...
    ray_data.path = ray_coords;                                                   % The geometric path vertices from TX to RX.
    ray_data.type = sprintf('%d-Refl', K);                          % A string label identifying the type of ray, e.g., "2-Refl".
    ray_data.dist = d_tot;                                               % The total distance the ray travels.
