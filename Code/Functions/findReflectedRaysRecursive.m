function valid_rays_found = findReflectedRaysRecursive(current_tx_pos, rx_pos, walls, num_reflections_remaining, processed_wall_indices, all_tx_images_pos, params)
...
%   all_tx_images_pos           - The sequence of image source positions generated so far.
...
%                         found from this recursive branch.
    ...
    if num_reflections_remaining == 0
        ...
        ray_coordinates = validateRayPath(rx_pos, walls, processed_wall_indices, all_tx_images_pos);
        
        % A non-empty result from validateRayPath means the path is unobstructed
        % and physically possible.
        if ~isempty(ray_coordinates)
            ray_data = calculatePhysicalProperties(ray_coordinates, processed_wall_indices, walls, params);  % If the path is valid, compute its physical characteristics (e.g., length, gain).
    ...
    for i = 1:length(walls)
        ...
        rays_from_branch = findReflectedRaysRecursive(new_image_source, rx_pos, walls, ...      % Pass the new virtual source as the starting point for the next level.
            ...
            [all_tx_images_pos, {new_image_source}], params);                                   % Append the new virtual source to the list of image positions.
