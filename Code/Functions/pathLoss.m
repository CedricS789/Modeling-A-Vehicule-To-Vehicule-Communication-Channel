...
%   sigma_L  - The calculated shadowing standard deviation (dB).
    ...
    PRX_sens_dBm = params.PRX_sens_dBm; % Receiver sensitivity from params
    L_max = PTX_dBm - PRX_sens_dBm; % Max total path loss
    ...
    for i = 1:length(reliabilities)
        ...
        % Calculate Fade Margin M based on reliability
        if reliability == 0.5
            M = 0; % For 50% reliability, outage is 50%, erfcinv(1) = 0
        else
            outage_prob = 1 - reliability;
            M = sigma_L * sqrt(2) * erfcinv(2 * outage_prob);
        end
        
        % Calculate L0_max using the calculated margin M
        L0_max = (L_max - M) + 2 * G_dBi;
        
        % Calculate Cell Range R
        R = d0 * 10^((L0_max - L0_d0_dB) / (10 * n));
